...
/*********************************************************************************************************
 *
 *	Object Name		: Chrono Mailer
 * 	Object Type		: SUPER CLASS
 *	Version			: 1.0
 *	Author			: Rico Dupuis (http://www.chronomedia.ca)
 * 	Date Created	: February 28th 2010, 15h00
 * 	Date Update		:
 * 	Date Update		:
 *
 *	Licence			: This class can used under Chrono Media terms of agreement
 *
 *	Description		: This object is using the common functions to send E-mail with option like
 *					  Bcc, Cc, Return-Path, Reply and Error tracking. All the returns error from the
 *					  SendMail() function can be logged and sent to an Admin E-mail.
 *
 *	Note			: Developed and tested on PHP v5.2.9 Server, MySQL 5.1 Server
 *					  Apache 2.2.11 and Internet Information Service (IIS) 5.1
 *
 *	Extra			  variable '' 	=
 *
 *
...
class ChronoMailer
{
    ...
    // System message
    public $msg = array(
    	"errorNoToDefined" 			=> "Error : no recipient selected for ",
    	"errorNoFromString" 		=> "Error, From should be inline",
    	"errorEmailValidate" 		=> "Error : wrong email ",
    	"errorPhpMailFn" 			=> "Error : mail() function returns error",
    	"successSentByMail" 		=> "Message successfully sent by mail()",
    	"errorSmtpSettings" 		=> "Error : missing required SMTP values ",
    	"errorSmtpConnect" 			=> "Error: can't connect to server",
    	"errorSmtpEhlo" 			=> "Error: greeting EHLO",
    	"errorSmtpServerDeniedAuth" => "server denies authorization",
    	"errorSmtpUserDenied" 		=> "user SMTP access failed ",
    	"errorSmtpPassword" 		=> "wrong password for SMTP",
    	"errorSmtpServerDeniedSend" => "command MAIL FROM through SMTP denied by server ",
    	"errorSmtpServerDeniedTo" 	=> "Server denied RCPT TO command through SMTP",
    	"errorSmtpServerDeniedData" => "server denied DATA command through SMTP",
    	"errorSmtpSend" 			=> "error sending message through SMTP",
    	"errorSmtpServerRSET" 		=> "Server denied RSET command",
    	"successSentByMail" 		=> "Message successfully sent through SMTP",
    	"infoLogDisabled" 			=> "Logging disabled. To create log file enable logging setting. Ex.:  \$mail->log_on(true);",
    );
    ...
    public function Body($text, $text_html = "", $alternative_text = '', $resource = 'webi')
    {
        ...
        elseif (strlen($alternative_text) && $text_html == 'text/html')
        {
    ...
    // 	=================================================================
    //	=				SET the correct mime type for attachment										=
    //	=================================================================
    ...
    public function BuildMail($resource = 'webi')
    {
        ...
        if (!is_array($this->sendto[$resource]) || !count($this->sendto[$resource]))
        {
            $this->status_mail['status'] = false;
            $this->status_mail['message'] = $this->msg['errorNoToDefined'].$resource;
            // return false;
        }
        ...
        $this->headers["X-Mailer"] = "Chrono_Mailer _v1.0";
    ...
    public function From($from)
    {
        ...
        {
            $this->status_mail['status'] = false;
            $this->status_mail['message'] = $this->msg['errorNoFromString'];
            return FALSE;
        }
    ...
    private function CheckAdresses($aad)
    {
        foreach ($aad as $key => $value)
        {
            ...
            {
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorEmailValidate'].$value;
                return FALSE;
            }
        }
    }
    ...
    public function Send()
    {
        ...
        if (!$this->smtp['on'])
        {
            foreach ($this->sendto as $key => $value)
            {
                ...
                {
                    $this->status_mail['status'] = false;
                    $this->status_mail['message'] = $this->msg['errorPhpMailFn'];
                }
                elseif ($this->status_mail['status'])
                {
                    ...
                    $this->status_mail['message'] = $this->msg['successSentByMail'];
                }
                if ($key != 'webi')
                {
        ...
        else
        {
            ...
            if (!$smtp_conn)
            {
                ...
                fclose($smtp_conn);
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorSmtpConnect'];
                return false;
            }
            ...
            if ($code != 250)
            {
                ...
                fclose($smtp_conn);
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorSmtpEhlo'];
                return false;
            }
            ...
            if ($code != 334)
            {
                ...
                fclose($smtp_conn);
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorSmtpServerDeniedAuth'];
                return false;
            }
            ...
            if ($code != 334)
            {
                ...
                fclose($smtp_conn);
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorSmtpUserDenied'];
                return false;
            }
            ...
            if ($code != 235)
            {
                ...
                fclose($smtp_conn);
                $this->status_mail['status'] = false;
                $this->status_mail['message'] = $this->msg['errorSmtpPassword'];
                return false;
            }
            foreach ($this->smtpsendto as $key_res => $value_res)
            {
                ...
                if ($code != 250)
                {
                    ...
                    fclose($smtp_conn);
                    $this->status_mail['status'] = false;
                    $this->status_mail['message'] = $this->msg['errorSmtpServerDeniedSend'];
                    return false;
                }
                foreach ($this->smtpsendto[$key_res] as $keywebi => $valuewebi)
                {
                    ...
                    if ($code != 250 AND $code != 251)
                    {
                        ...
                        fclose($smtp_conn);
                        $this->status_mail['status'] = false;
                        $this->status_mail['message'] = $this->msg['errorSmtpServerDeniedTo'];
                        return false;
                    }
                }
                ...
                if ($code != 354)
                {
                    ...
                    fclose($smtp_conn);
                    $this->status_mail['status'] = false;
                    $this->status_mail['message'] = $this->msg['errorSmtpServerDeniedData'];
                    return false;
                }
                ...
                if ($code != 250)
                {
                    ...
                    fclose($smtp_conn);
                    $this->status_mail['status'] = false;
                    $this->status_mail['message'] = $this->msg['errorSmtpSend'];
                    return false;
                }
                ...
                if ($code != 250)
                {
                    ...
                    fclose($smtp_conn);
                    $this->status_mail['status'] = false;
                    $this->status_mail['message'] = $this->msg['errorSmtpServerRSET'];
                    return false;
                }
                if ($key_res != 'webi')
                {
    ...
    {
        if (!$this->log_on)
            return $this->msg['infoLogDisabled'];
